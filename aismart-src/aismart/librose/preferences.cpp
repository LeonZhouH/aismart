...
{
	scoped_istream stream = preprocess_file(get_prefs_file());
	read(prefs, *stream);
...
void write_preferences()
{
	try {
		...
		write(out, prefs);
		write_file(get_prefs_file(), out.str().c_str(), out.str().size());
...
std::string version()
{
	return preferences::get("version");
}
...
{
	return 480 * gui2::twidget::hdpi_scale;
}

int min_allowed_height()
{
	return 320 * gui2::twidget::hdpi_scale;
}
...
std::pair<int,int> resolution()
{
	...
	if (!x.empty() && !y.empty()) {
		width = std::max(atoi(x.c_str()) * gui2::twidget::hdpi_scale, min_allowed_width());
		height = std::max(atoi(y.c_str()) * gui2::twidget::hdpi_scale, min_allowed_height());
