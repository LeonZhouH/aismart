...
ttree::ttree(const std::vector<tnode_definition>& node_definitions)
	: tscroll_container(2)
	, grid2_(nullptr)
...
int ttree::gc_lookup_find(const ttree_node* node) const
{
	...
	if (hit_at == twidget::npos) {
		...
		VALIDATE(hit_at != twidget::npos, null_str);
	}
	return hit_at;
}
...
void ttree::gc_insert_or_erase_row(const int row, const bool insert, ttree_node** nodes, const int count)
{
	...
	if (row <= gc_next_precise_at_) {
		...
		if (insert && gc_next_precise_at_ < original_gc_next_pricise_at + count) {
			// when insert, inserted node maybe gc_height_ == twidget::npos, then bottom part require from precise to estimated. 
	...
	if (!insert && gc_first_at_ != twidget::npos) {
		...
		for (int n = 0; n < count; n ++) {
			...
			// if gc_last_at_ is in [0, n], will twidget::npos it. if not, 1)if gc_first_at_ in [0, n], decrease it. 2) do nothing.
			ttree_node* widget = nodes[n];
			widget->gc_distance_ = twidget::npos;
			if (gc_first_at_ != twidget::npos) {
		...
		int first_distance = twidget::npos;
		if (gc_first_at_ < row) {
			if (gc_last_at_ >= row) {
			...
			if (gc_last_at_ < row + count) {
				gc_last_at_ = gc_first_at_ = twidget::npos;
			} else {
				gc_last_at_ -= count;
				gc_first_at_ = row;
		...
		// update gc_distance_
		if (first_distance != twidget::npos && gc_first_at_ > gc_next_precise_at_) {
...
tpoint ttree::tgrid2::fill_placeable_width(const int width)
{
...
void ttree::tgrid2::place(const tpoint& origin, const tpoint& size)
{
	...
	VALIDATE(tree_.gc_first_at_ != twidget::npos, null_str);
	VALIDATE(tree_.gc_current_content_width_ != twidget::npos, null_str);
